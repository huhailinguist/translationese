...
class StanfordCoreNLP:
    ...
    def tregex(self, sentence, pattern):
        ...
        r_dict = self._request(url=tregex_url, pattern=pattern,
    ...
    def tokensregex(self, sentence, pattern):
        ...
        r_dict = self._request(url=tokensregex_url, pattern=pattern,
    ...
    def semgrex(self, sentence, pattern):
        ...
        r_dict = self._request(url=semgrex_url, pattern=pattern,
                               annotators="tokenize,ssplit,depparse",
                               data=sentence)
        return r_dict

    # ---------- added ------------------
    def ssplit(self, sentences):
        r_dict = self._request('ssplit', sentences)
        res = []
        if self.lang == 'zh':
            for s in r_dict['sentences']:
                oneSent = ''.join([token['word'] for token in s['tokens']])
                res.append(oneSent)
        else:
            for s in r_dict['sentences']:
                oneSent = ' '.join([token['word'] for token in s['tokens']])
    ...
    def pos_tag(self, sentence):
        ...
        for s in r_dict['sentences']:
            for token in s['tokens']:
                # ---------- added ------------------
                # now in chinese mode, the words will be shown correctly.
                if self.lang == 'zh':  # when it's zh, token['originalText']=''
    ...
    def dependency_parse(self, sentence):
        ...
        return [(dep['dep'], dep['governor'], dep['dependent']) \
                for s in r_dict['sentences'] for dep in \
                s['basicDependencies']]
    ...
    def _request(self, annotators=None, data=None, *args, **kwargs):
        ...
        # chinese ssplit: ssplit.boundaryTokenRegex = [.。]|[!?！？]+
        # or: ssplit.newlineIsSentenceBreak = always
        # or: start the server separately with properties!
        if self.lang == 'zh':
            properties = {
            'annotators': annotators, 'outputFormat': 'json',
        ...
        if 'url' in kwargs:  # for tregex
            r = requests.post(kwargs['url'], params=params, data=data, headers={'Connection': 'close'})
        else:
